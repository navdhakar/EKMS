...
mongodb.connect(config.settings.database.connection_string, {}, (err, mdb) => {
  ...
  insertKB(mdb, (KBerr, report) => {
    insertUsers(mdb, (Usererr, report) => {
      if (KBerr || Usererr) {
        console.log(
          "There was an error upgrading to MongoDB. Check the console output"
        );
      } else {
  ...
  ndb.loadDatabase((err) => {
    ...
    ndb.find({}, (err, docs) => {
      if (docs.length === 0) {
        ...
        console.log(
          "Loaded " + docs.length + " article(s) data from the NeDB database"
        );
        console.log("");
      }
  ...
  ndb.loadDatabase((err) => {
    ...
    ndb.find({}, (err, docs) => {
      if (docs.length === 0) {
        ...
        console.log(
          "Loaded " + docs.length + " user(s) data from the NeDB database"
        );
        console.log("");
      }
