$(document).ready(function () {
          ...
          $.ajax({
            ...
            if (response.length === 0) {
              ...
              $(".searchResultList").append(
                '<li class="list-group-item list-group-heading">Search results</li>'
              );
  ...
  if (config.add_header_anchors === true) {
    $(
      ".body_text > h1, .body_text > h2, .body_text > h3, .body_text > h4, .body_text > h5"
    ).each(function () {
      $(this).attr("id", convertToSlug($(this).text()));
      $(this).prepend(
        '<a class="headerAnchor" href="#' +
          convertToSlug($(this).text()) +
          '">#</a> '
      );
    });
  }
  ...
  if ($("#editor").length) {
    ...
    if (preview !== null) {
      ...
      preview.onscroll = function () {
        ...
        var move =
          (simplemde.codemirror.getScrollInfo().height -
            simplemde.codemirror.getScrollInfo().clientHeight) *
          ratio;
  ...
  $(document).on("click", "#frm_edit_kb_save", function (e) {
    ...
    if ($("#versionSidebar").length) {
      ...
      if ($("#frm_kb_edit_reason").val() === "") {
        show_notification(
          "Please enter a reason for editing article",
          "danger"
        );
  ...
  function convertTextAreaToMarkdown(firstRender) {
    ...
    if (typeof mermaid !== "undefined" && config.mermaid) {
      ...
      var defFenceRules = mark_it_down.renderer.rules.fence.bind(
        mark_it_down.renderer.rules
      );
      mark_it_down.renderer.rules.fence = function (
        tokens,
        idx,
        options,
        env,
  ...
  $("#btn_articles_filter").click(function () {
    window.location.href =
      $("#app_context").val() +
      "/articles/" +
      encodeURIComponent($("#article_filter").val());
  });
